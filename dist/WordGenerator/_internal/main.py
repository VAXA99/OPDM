...
custom_jobplace = st.text_input("💼 Введите место работы:", value="ГБОУ Школа №1343")
...
zip_name = st.text_input("📝 Имя папки/ZIP-архива (без .zip):", value="generated_docs")
...
if save_to_fs:
    ...
    final_save_dir = st.text_input("Подпапка для DOCX (создастся автоматически, оставьте пустым для корня):", value=zip_name, key="docx_subdir")
    if final_save_dir:
        target_dir = selected_path / final_save_dir
    else:
...
if save_zip_to_fs:
    ...
    zip_final_save_dir = st.text_input("Подпапка для ZIP (создастся автоматически, оставьте пустым для корня):", value=zip_name, key="zip_subdir")
    if zip_final_save_dir:
        zip_target_dir = zip_selected_path / zip_final_save_dir
    else:
        zip_target_dir = zip_selected_path
    st.info(f"ZIP-архив сохранится в: {zip_target_dir}")
        ...
        try:
            ...
            if not cols:
                ...
                if save_zip_to_fs:
                    ...
                    zip_target_path = zip_target_dir / f"{zip_name}.zip"
                    with open(zip_target_path, "wb") as f:
                        f.write(zip_buffer)
                    zip_fs_success = str(zip_target_path)
